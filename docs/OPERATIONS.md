...
All API examples assume `-u admin:<INDEXER_ADMIN_PASSWORD>` and one of the indexer hosts.
...
`configs/indexer/ism_policy_alerts.json` and `configs/indexer/ism_policy_archives.json`.
...
curl -k -u admin:<INDEXER_ADMIN_PASSWORD> -X PUT \
  "https://<INDEXER_01_IP>:9200/_plugins/_ism/policies/wazuh-alerts-policy" \
  -H "Content-Type: application/json" \
  --data-binary @configs/indexer/ism_policy_alerts.json

# Archives policy (30 day retention)
curl -k -u admin:<INDEXER_ADMIN_PASSWORD> -X PUT \
  "https://<INDEXER_01_IP>:9200/_plugins/_ism/policies/wazuh-archives-policy" \
  -H "Content-Type: application/json" \
  --data-binary @configs/indexer/ism_policy_archives.json
```
...
curl -k -u admin:<INDEXER_ADMIN_PASSWORD> \
  "https://<INDEXER_01_IP>:9200/_plugins/_ism/policies" | grep -o '"_id" : "[^"]*"'
```
...
`configs/indexer/index_template_alerts.json`):
...
curl -k -u admin:<INDEXER_ADMIN_PASSWORD> "https://<INDEXER_01_IP>:9200/_nodes/stats?pretty"
...
curl -k -u admin:<INDEXER_ADMIN_PASSWORD> "https://<INDEXER_01_IP>:9200/_cat/recovery?v"
curl -k -u admin:<INDEXER_ADMIN_PASSWORD> "https://<INDEXER_01_IP>:9200/_cluster/pending_tasks?pretty"
```
...
```bash
curl -k -u admin:<INDEXER_ADMIN_PASSWORD> -X PUT \
  "https://<INDEXER_01_IP>:9200/_snapshot/wazuh_backup?verify=false" \
...
```bash
curl -k -u admin:<INDEXER_ADMIN_PASSWORD> -X PUT \
  "https://<INDEXER_01_IP>:9200/_snapshot/wazuh_backup/daily-$(date +%F)?wait_for_completion=true" \
  -H 'Content-Type: application/json' -d '{
...
```bash
curl -k -u admin:<INDEXER_ADMIN_PASSWORD> -X POST \
  "https://<INDEXER_01_IP>:9200/_snapshot/wazuh_backup/<SNAPSHOT_NAME>/_restore" \
  -H 'Content-Type: application/json' -d '{
...
```bash
curl -k -u admin:<INDEXER_ADMIN_PASSWORD> -X PUT "https://<INDEXER_01_IP>:9200/_all/_settings" \
  -H 'Content-Type: application/json' -d '{
...
* Index retention policy (`configs/indexer/ism_policy_alerts.json`, `configs/indexer/ism_policy_archives.json`).
...
The checks in this section are also bundled as a script. Copy `scripts/validate_cluster.sh`
to any server node and run it to walk the indexer health, the server cluster, the API, and
the Filebeat output in one pass. Run it after every stage rather than only at the end, since
a layer that breaks is much cheaper to find immediately than three stages later.

Run top to bottom. Each layer must pass before trusting the next.

### Indexer cluster
```bash
curl -k -u admin:<INDEXER_ADMIN_PASSWORD> "https://<INDEXER_01_IP>:9200/_cluster/health?pretty"   # green, 3 nodes
curl -k -u admin:<INDEXER_ADMIN_PASSWORD> "https://<INDEXER_01_IP>:9200/_cat/nodes?v"             # 3 nodes, one manager
```
...
curl -sk -u wazuh-wui:wazuh-wui \
  -X POST https://<MASTER_IP>:55000/security/user/authenticate | python3 -m json.tool | head -3
```
...
### Agents and groups
```bash
sudo /var/ossec/bin/agent_control -l         # 5 agents Active
...
```bash
# On ubuntu-agent-01 (<UBUNTU_AGENT_01_IP>)
for i in {1..6}; do
...
```bash
curl -k -u admin:<INDEXER_ADMIN_PASSWORD> \
  "https://<INDEXER_01_IP>:9200/wazuh-alerts-*/_search?pretty" \
  -H "Content-Type: application/json" -d '{
    "size": 3,
...
* 5 agents active: 2 in `linux`, 2 in `windows`, and the domain controller in `win`, all enrolled through the load balancer.
